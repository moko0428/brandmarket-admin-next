...
export default function Drawer({ open, onClose, children }: DrawerProps) {
  ...
  useEffect(() => {
    ...
    const up = () => {
      ...
      if (final >= CLOSE) {
        ...
        /* 최소/최대 높이 제한 */
        const currentHeight = MIN_HEIGHT - final; // final이 음수이므로
  ...
  }, [dragging, dragY, offsetY, onClose, CLOSE, MIN_HEIGHT, MAX_HEIGHT]);
  ...
  return (
    <>
      ...
      >
        {/* 핸들바 (드래그 전용 영역) */}
        <div className="flex justify-center py-3" onPointerDown={onPointerDown}>
